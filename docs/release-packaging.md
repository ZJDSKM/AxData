...
发布标签必须和 `packages/axdata-sdk/pyproject.toml` 里的版本一致。例如当前版本是 `0.1.0`，则 GitHub Release 标签应为 `v0.1.0` 或 `0.1.0`。PyPI 不允许覆盖已发布的同名同版本文件；如果要重新发布，需要先提升版本号。
...
4. GitHub Release 标签等于当前包版本，例如 `v0.1.0`。
