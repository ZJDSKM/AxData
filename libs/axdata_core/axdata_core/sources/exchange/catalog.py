...
INTERFACES: dict[str, SourceRequestInterface] = {
    ...
    "stock_prbook_em": SourceRequestInterface(
        name="stock_prbook_em",
        display_name_zh="定期报告预约披露（东财，沪深统一）",
        source_code="exchange",
        source_name_zh="交易所",
        category="基础数据",
        request_mode="source_request",
        first_stage_strategy=(
            "临时请求东方财富 datacenter 定期报告预约披露接口"
            "（RPT_PUBLIC_BS_APPOIN），沪深两市统一，默认不入库。"
        ),
        source_ability="Eastmoney periodic report appointment",
        description=(
            "Return periodic report appointment dates from Eastmoney datacenter "
            "(both SSE and SZSE). REPORT_TYPE: 1=一季报 2=半年报 3=三季报 4=年报."
        ),
        parameters=(
            RequestParameter(
                name="code",
                dtype="string",
                required=True,
                description="Stock code, e.g. 600101 or 000001.",
                description_zh="股票代码（沪深通用），例如 600101 或 000001。",
            ),
        ),
        fields=(
            RequestField(name="instrument_id", dtype="string",
                         description="AxData unified security code.",
                         description_zh="AxData 统一证券代码。"),
            ...
            RequestField(name="exchange", dtype="string",
                         ...
                         description_zh="交易所（SSE/SZSE/BSE）。"),
            RequestField(name="bulletin_type", dtype="string",
                         description="Report type code 1-4.",
                         description_zh="报告类型码（1-4）。"),
            RequestField(name="report_type", dtype="string",
                         description="一季报/半年报/三季报/年报.",
                         description_zh="报告类型名称。"),
            ...
            RequestField(name="change_date_1", dtype="string",
                         description="First change date.",
                         description_zh="第一次变更日。"),
            RequestField(name="change_date_2", dtype="string",
                         description="Second change date.",
                         description_zh="第二次变更日。"),
            RequestField(name="change_date_3", dtype="string",
                         description="Third change date.",
                         description_zh="第三次变更日。"),
            RequestField(name="actual_date", dtype="string",
                         description="Actual publish date.",
                         description_zh="实际披露日。"),
        ),
        example=RequestExample(
    ...
    "stock_performance_forecast_em": SourceRequestInterface(
        name="stock_performance_forecast_em",
        display_name_zh="业绩预告（东财结构化）",
        source_code="exchange",
        source_name_zh="交易所",
        category="基础数据",
        request_mode="source_request",
        first_stage_strategy=(
            "临时请求东方财富 datacenter 业绩预告接口"
            "（RPT_PUBLIC_OP_NEWPREDICT，净利润区间/变动幅度结构化），默认不入库。"
        ),
        source_ability="Eastmoney performance forecast",
        description=(
            "Return structured performance forecasts (net profit range, "
            "change amplitude). Only companies with material changes issue "
            "forecasts per exchange rules."
        ),
        parameters=(
            RequestParameter(
                name="code",
        ...
        fields=(
            ...
            RequestField(name="exchange", dtype="string",
                         description="SSE/SZSE/BSE.",
                         description_zh="交易所。"),
            RequestField(name="notice_date", dtype="string",
                         description="Announcement date YYYYMMDD.",
                         description_zh="公告日期 YYYYMMDD。"),
            RequestField(name="report_date", dtype="string",
                         description="Report period end date YYYYMMDD.",
                         description_zh="报告期截止日 YYYYMMDD。"),
            RequestField(name="finance_item", dtype="string",
                         description="Predicted finance item.",
                         description_zh="预测财务指标。"),
            RequestField(name="predict_amount_lower", dtype="number",
                         description="Net profit lower bound (yuan).",
                         description_zh="预测净利润下限（元）。"),
            RequestField(name="predict_amount_upper", dtype="number",
                         description="Net profit upper bound (yuan).",
                         description_zh="预测净利润上限（元）。"),
            RequestField(name="adjust_amp_lower", dtype="number",
                         description="Change amplitude lower bound (%).",
                         description_zh="变动幅度下限（%）。"),
            RequestField(name="adjust_amp_upper", dtype="number",
                         description="Change amplitude upper bound (%).",
                         description_zh="变动幅度上限（%）。"),
            RequestField(name="content", dtype="string",
                         description="Forecast content text.",
                         description_zh="业绩预告内容。"),
        ),
        example=RequestExample(
            request={"params": {"code": "600101"}, "persist": False},
            response=(
                {
                    ...
                    "symbol": "600101",
                    "exchange": "SSE",
                    "notice_date": "20231017",
                    "report_date": "20230930",
                    "finance_item": "归属于上市公司股东的净利润",
                    "predict_amount_lower": 185770000.0,
                    "predict_amount_upper": 185770000.0,
                    "adjust_amp_lower": 26.0,
                    "adjust_amp_upper": 26.0,
                    "content": "预计2023年1-9月…盈利约18,577万元,同比上年增长26%",
                },
            ),
        ),
    ),
    "stock_performance_express_em": SourceRequestInterface(
        name="stock_performance_express_em",
        display_name_zh="业绩快报（东财结构化）",
        source_code="exchange",
        source_name_zh="交易所",
        category="基础数据",
        request_mode="source_request",
        first_stage_strategy=(
            "临时请求东方财富业绩快报接口（RPT_FCI_PERFORMANCEE，净利/"
            "营收/同比结构化），默认不入库。"
        ),
        source_ability="Eastmoney performance express",
        description=(
            "Return structured performance express (net profit, revenue, "
            "YoY changes, notice date)."
        ),
        parameters=(
            RequestParameter(
                name="code",
        ...
        fields=(
            ...
            RequestField(name="symbol", dtype="string",
                         ...
                         description_zh="交易所六位代码。"),
            RequestField(name="exchange", dtype="string",
                         description="SSE/SZSE.",
                         description_zh="交易所。"),
            RequestField(name="notice_date", dtype="string",
                         description="Express notice date YYYYMMDD.",
                         description_zh="快报公告日期 YYYYMMDD。"),
            RequestField(name="report_date", dtype="string",
                         description="Report period end date YYYYMMDD.",
                         description_zh="报告期截止日 YYYYMMDD。"),
            RequestField(name="net_profit", dtype="number",
                         description="Parent company net profit (yuan).",
                         description_zh="归母净利润（元）。"),
            RequestField(name="net_profit_yoy", dtype="number",
                         description="Net profit YoY change (%).",
                         description_zh="净利同比（%）。"),
            RequestField(name="revenue", dtype="number",
                         description="Total revenue (yuan).",
                         description_zh="营业收入（元）。"),
            RequestField(name="revenue_yoy", dtype="number",
                         description="Revenue YoY change (%).",
                         description_zh="营收同比（%）。"),
            RequestField(name="roe", dtype="number",
                         description="Weighted ROE (%).",
                         description_zh="加权 ROE（%）。"),
            RequestField(name="data_type", dtype="string",
                         description="Report type label.",
                         description_zh="报告类型标签。"),
        ),
        example=RequestExample(
            request={"params": {"code": "600101"}, "persist": False},
            response=(
                {
                    ...
                    "symbol": "600101",
                    "exchange": "SSE",
                    "notice_date": "20260722",
                    "report_date": "20260630",
                    "net_profit": 53146200.0,
                    "net_profit_yoy": -27.91,
                    "revenue": 1558120900.0,
                    "revenue_yoy": 2.25,
                    "roe": 1.71,
                    "data_type": "2026年 半年报",
                },
